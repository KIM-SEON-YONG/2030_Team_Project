...
@WebServlet("/PolicyWeddingSrcCon")
public class PolicyWeddingSrcCon extends HttpServlet { 
	private static final long serialVersionUID = 1L;
	 ...
	 response) throws ServletException, IOException {
		...
		if(regionParam.length() > 0) {
			...
			RequestDispatcher rd = request.getRequestDispatcher("policy_wedding.jsp");
		...
		List<PolicyDTO> selecWeddingList = dao.selecWeddingList(regionList);
		
		
		// 결과처리 =>frontController 연결 시 redirect 형태로 변경해야됨
		HttpSession session = request.getSession();
		// 선택한 지역에 대한 복지혜택이 있는지 여부 확인
		if(selecWeddingList != null && !selecWeddingList.isEmpty()) {
			System.out.println("마지막 jsp 리턴하기전 배열개수:"+selecWeddingList.size());
			
				// JSON 변환 라이브러리 - GSON 사용
				Gson gson = new Gson();
				String json = gson.toJson(selecWeddingList);
...
//				}
//				// foreach문 사용
				for(PolicyDTO policy : selecWeddingList) {
					System.out.println("마지막 jsp로 리턴하기전: "+policy.getWelfare_title());
				}
			
				session.setAttribute("selecWeddingList", selecWeddingList);
